...
Page({
  ...
  data: {
    ...
    detail: [],
    addressIndexArr: ['北京', '安徽', "福建", "甘肃", "广东", "广西", "贵州", "海南", "河北", "河南", "黑龙江", "湖北", "湖南", "吉林", "江苏", "江西", "辽宁", "内蒙古", "宁夏", "青海", "山东", "山西", "陕西", "上海", "四川", "天津", "西藏", "新疆", "云南", "浙江", "重庆", "香港", "澳门", "台湾"],
    addressIndex:0,
    addressFromArr:["陕西省 - 西安市"],
    addressFromIndex:0,
    carTypeArr:["小型汽车"],
    carTypeIndex:0,
    isShow: false,
  ...
  inputlinsener:function(e){
    ...
    switch (val) {
      case 'consignmentFrom':
        this.setData({
          consignmentFromValue: e.detail.value
        })
        break;
      case 'address':
        this.setData({
  ...
  submitOrder:function(e){
    ...
    var self = this;
    // if (!self.checkPhoneNum(self.data.contactNumber))
    //   return;
    ...
    //var addressTo = self.data.addressIndexArr[self.data.addressIndex];  
    var addressTo = self.data.multiArray[1][self.data.multiIndex[1]]+"市"
    var subDriArr = {
      "carType": self.data.carTypeArr[0],
      "consignmentFromValue": "西安市",
      "consignmentToValue": addressTo,
      "resolveTime": self.data.dates,
      "contactValue": self.data.contactValue,
      "contactPhone": self.data.isRegPhone
      //"inviteTelphone": self.data.refereePhoneNum
  };
console.log(subDriArr)
    console.info(self.data.isRegPhone)
    if(self.data.isRegPhone!=null){
      wx.showModal({
        title: '确认',
        content: "西安市 --> " + addressTo,
            ...
            wx.request({
              ...
              success: function (res) {
                ...
                if (true) {
                  wx.showModal({
                    title: '',
                    content: "我们已经收到您的请求，我们尽快与您联系，请保持手机畅通",
                    showCancel: false,
                    success: function (res) {
                      if (res.confirm) {
                        wx.navigateBack()
  ...
  bindTargetAddressChange: function(e){
    console.log(e)
    this.setData({
      addressIndex:e.detail.value
    })
  },
  bindTargetAddressFromChange: function(e){
    this.setData({
      addressIndexFrom: e.detail.value
    })
  },
  bindCarTypeIndexChange: function(e){
    this.setData({
